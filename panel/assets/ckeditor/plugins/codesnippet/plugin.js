 ...
 ( function() {
		...
		beforeInit: function( editor ) {
			 ...
			 */
			 this.setHighlighter = function( highlighter ) {
				...
				editor._.codesnippet.langsRegex = new RegExp( '(?:^|\\s)code-(' +
	...
	function registerWidget( editor ) {
		...
		editor.widgets.add( 'codeSnippet', {
			...
			template: '<pre class="' + codeClass + '"><kod></kod><code></code></pre>',
			dialog: 'codeSnippet',
			...
			data: function() {
				...
				this.parts.kod.setHtml( newData.lang);
				...
				if ( oldData && newData.lang != oldData.lang )
					this.parts.pre.removeClass( 'code-' + oldData.lang );

				// Lang needs to be specified in order to apply formatting.
				if ( newData.lang ) {
					// Apply new .language-* class.
					this.parts.pre.addClass( 'code-' + newData.lang );
					this.parts.pre.addClass( 'code');
